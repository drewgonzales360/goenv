...
func (config *Config) WarnOnMissingPath() {
	bin := config.GoenvRootDirectory + "/bin"
	if path := os.Getenv("PATH"); !strings.Contains(path, bin) {
		Info(fmt.Sprintf("%s is not in your PATH", bin))
		Info(fmt.Sprintf("export PATH=%s:$PATH", bin))
	}
}

func (config *Config) VersionInstallPath(goVersion *semver.Version) string {
	return path.Join(config.GoenvInstallDirectory, goVersion.String())
}
